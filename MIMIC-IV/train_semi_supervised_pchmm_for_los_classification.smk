...
RESULTS_FEAT_PER_TSTEP_PATH="results/PCHMM/los_classification"
CLF_TRAIN_TEST_SPLIT_PATH = "/cluster/tufts/hugheslab/datasets/MIMIC-IV/ordinal_los_prediction/"
PROJECT_REPO_DIR = "PC-HMM"

rule train_and_evaluate_classifier_many_hyperparams:
    input:
        [os.path.join(RESULTS_FEAT_PER_TSTEP_PATH,"semi-supervised-pchmm-lr={lr}-seed={seed}-init_strategy={init_strategy}-batch_size={batch_size}-min_los={min_los}-predictor_l2_penalty={predictor_l2_penalty}-n_states={n_states}-lamb={lamb}.csv").format(lr=lr, seed=seed, init_strategy=init_strategy, batch_size=batch_size, min_los=min_los, n_states=n_states, lamb=lamb, predictor_l2_penalty=predictor_l2_penalty) for lr in config['lr'] for seed in config['seed'] for init_strategy in config['init_strategy'] for batch_size in config['batch_size'] for min_los in config['min_los'] for n_states in config['n_states'] for lamb in config['lamb'] for predictor_l2_penalty in config['predictor_l2_penalty']]
        
rule train_and_evaluate_classifier:
    input:
        script=os.path.join(PROJECT_REPO_DIR, 'main_mimic_semi_supervised.py'),
        x_train_np=os.path.join(CLF_TRAIN_TEST_SPLIT_PATH, 'los_geq_{min_los}_days_prediction', 'X_train.npy'),
        ...
        y_train_np=os.path.join(CLF_TRAIN_TEST_SPLIT_PATH, 'los_geq_{min_los}_days_prediction','y_train.npy'),
    ...
    params:
        ...
        fn_prefix="semi-supervised-pchmm-lr={lr}-seed={seed}-init_strategy={init_strategy}-batch_size={batch_size}-min_los={min_los}-predictor_l2_penalty={predictor_l2_penalty}-n_states={n_states}-lamb={lamb}"
    
    output:
        os.path.join(RESULTS_FEAT_PER_TSTEP_PATH, "semi-supervised-pchmm-lr={lr}-seed={seed}-init_strategy={init_strategy}-batch_size={batch_size}-min_los={min_los}-predictor_l2_penalty={predictor_l2_penalty}-n_states={n_states}-lamb={lamb}.csv")
