...
#     X_test_unlabelled = X_test[~labelled_inds_te]
    ...
    for d in range(D):
        ...
        # forward fill
        X_train_labelled[:, :, d] = ffill(X_train_labelled[:, :, d])
        X_valid_labelled[:, :, d] = ffill(X_valid_labelled[:, :, d])
        X_test_labelled[:, :, d] = ffill(X_test_labelled[:, :, d])
...
# #         # zscore normalization
    ...
    train_unlabelled_aug_data = train_unlabelled_data + 0.1*torch.randn_like(train_unlabelled_data)
    ...
    if len(labeled_trainloader)==0:
        ...
        unlabeled_trainloader = data.DataLoader(train_unlabeled_set, batch_size=len(train_labeled_set), shuffle=True, num_workers=0, drop_last=True)
...
#     cudnn.benchmark = True
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        if args.perc_labelled=='100':# fully supervised
            train_loss = train_with_bce(labeled_trainloader, model, optimizer, criterion, ema_optimizer)
        else:
        ...
        train_loss, train_auprc, train_auroc = validate(full_train_loader, ema_model, criterion, epoch, use_cuda, mode='Train Stats')
...
def train_with_bce(labeled_trainloader, model, optimizer, criterion, ema_optimizer):
