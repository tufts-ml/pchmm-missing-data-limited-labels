...
#             den=1
    ...
    full_labeled_trainloader = data.DataLoader(train_labeled_set, 
                                               batch_size=len(train_labeled_set), shuffle=False, num_workers=0)
...
#     cudnn.benchmark = True
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        train_loss, train_auprc, train_auroc = validate(full_labeled_trainloader, 
                                                        ema_model, criterion, epoch, use_cuda, mode='Train Stats')
        valid_loss, valid_auprc, valid_auroc = validate(val_loader, 
                                                        ema_model, criterion, epoch, use_cuda, mode='Valid Stats')
        test_loss, test_auprc, test_auroc = validate(test_loader, 
                                                     ema_model, criterion, epoch, use_cuda, mode='Test Stats ')
