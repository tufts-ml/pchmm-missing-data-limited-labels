...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '--dataset_raw_path',
        default=None,
        help='Path to the top folder of mimic-iv in-hospital mortality dataset')
    parser.add_argument(
        '--output_dir',
        default=None,
    ...
    data_dir = os.path.join(root_dir, 'data_labs_vitals_icu_mortality_prediction', 'features')
    ...
    outcomes_df = pd.read_csv('/cluster/tufts/hugheslab/prath01/datasets/MIMIC-IV/physionet.org/files/mimiciv/MIMIC-IV-Data-Pipeline/data_labs_vitals_icu_mortality_prediction/cohort/cohort_icu_mortality.csv.gz')
    ...
    if args.keep_first_24_hours_only=='true':
        ...
        keep_inds = data_df['hours_from_admission']<24.0
...
#     data_df = data_df.rename(columns={'valuenum':'value'})
    ...
    for ii, lv in enumerate(unique_labs_vitals): 
        curr_df = data_df.loc[data_df.label==lv, ['stay_id', 'hours_from_admission', 'value']].rename(columns={'value' : lv}) 
        if ii==0: 
...
#     feature_cols = keep_columns + ['bmi']
    ...
    final_df['timestamp'] = pd.to_datetime(final_df['admission_timestamp'])+pd.to_timedelta(final_df.hours_from_admission, unit='h') 
