'''
Train MixMatch ON MIMIC Inhospital Mortality Task

Usage
-----
snakemake --cores 1 --snakefile train_semi_supervised_MixMatch_for_los_classification.smk train_and_evaluate_classifier_many_hyperparams

Schedule as slurm jobs
----------------------
$ snakemake --snakefile train_semi_supervised_MixMatch_for_los_classification.smk --profile ../../utils/profiles/hugheslab_cluster/ train_and_evaluate_classifier_many_hyperparams
...
rule train_and_evaluate_classifier:
    ...
    params:
        ...
        fn_prefix="MixMatch-lr={lr}-seed={seed}-min_los={min_los}-batch_size={batch_size}-perc_labelled={perc_labelled}"
    
    output:
        os.path.join(RESULTS_FEAT_PER_TSTEP_PATH, "MixMatch-lr={lr}-seed={seed}-min_los={min_los}-batch_size={batch_size}-perc_labelled={perc_labelled}.csv")
