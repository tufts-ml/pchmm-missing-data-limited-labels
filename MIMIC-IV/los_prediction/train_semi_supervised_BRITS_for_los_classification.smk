...
Usage
-----
snakemake --cores 1 --snakefile train_semi_supervised_BRITS_for_los_classification.smk train_and_evaluate_classifier_many_hyperparams

Schedule as slurm jobs
----------------------
$ snakemake --snakefile train_semi_supervised_BRITS_for_los_classification.smk --profile ../../utils/profiles/hugheslab_cluster/ train_and_evaluate_classifier_many_hyperparams
...
RESULTS_FEAT_PER_TSTEP_PATH=os.path.abspath("../results/BRITS/los_classification")
CLF_TRAIN_TEST_SPLIT_PATH = "/cluster/tufts/hugheslab/datasets/MIMIC-IV/ordinal_los_prediction/"


rule train_and_evaluate_classifier_many_hyperparams:
    input:
        [os.path.join(RESULTS_FEAT_PER_TSTEP_PATH,"final_perf_BRITS-lr={lr}-seed={seed}-min_los={min_los}-batch_size={batch_size}-perc_labelled={perc_labelled}.csv").format(lr=lr, seed=seed, min_los=min_los, batch_size=batch_size, perc_labelled=perc_labelled) for lr in config['lr'] for seed in config['seed'] for min_los in config['min_los'] for batch_size in config['batch_size'] for perc_labelled in config['perc_labelled']]
        
rule train_and_evaluate_classifier:
    input:
        script=os.path.join(PROJECT_REPO_DIR, 'BRITS', 'main.py'),
    ...
    params:
        ...
        fn_prefix="final_perf_BRITS-lr={lr}-seed={seed}-min_los={min_los}-batch_size={batch_size}-perc_labelled={perc_labelled}"
    
    output:
        os.path.join(RESULTS_FEAT_PER_TSTEP_PATH, "final_perf_BRITS-lr={lr}-seed={seed}-min_los={min_los}-batch_size={batch_size}-perc_labelled={perc_labelled}.csv")
