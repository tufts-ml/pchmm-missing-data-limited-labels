...
#     parser.add_argument('--random_state', required=False, type=int, default=20190206)
    ...
    if 'start' in df.columns:
        ...
        df_timesorted = df.sort_values(by=id_cols + ['admission_timestamp', 'start', 'stop']) 
    else:
        id_cols = parse_id_cols(data_dict)       
        df_timesorted = df.sort_values(by=['admission_timestamp'] + id_cols + ['hours_from_admission']) 
    
    del df
        
    # set the first 3 years of admissions for training
    train_admission_ts_start = '2110-01-01 00:00:00'
    valid_admission_ts_start = '2160-01-01 00:00:00'
    test_admission_ts_start = '2177-01-01 00:00:00'
    ...
    train_df = train_df.sort_values(by=id_cols+['start', 'stop']) 
    ...
    valid_df = valid_df.sort_values(by=id_cols+['start', 'stop'])
    
    
    
    test_df = df_timesorted[(df_timesorted['admission_timestamp']>=test_admission_ts_start)]
    test_df = test_df.sort_values(by=id_cols+['start', 'stop'])
