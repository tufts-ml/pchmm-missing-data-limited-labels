...
#     ignore_states = 5
    kmeans = KMeans(n_clusters=n_states+2, n_init=10).fit(stacked_means_across_classes)
    init_means = kmeans.cluster_centers_
    sorted_init_means = init_means[np.argsort(init_means[:, 0]), :]
    init_means = sorted_init_means[1:n_states+1]
    ...
    model.fit(data, steps_per_epoch=5, epochs=500, 
    ...
    perf_dict['valid AUC_mean']=0
    perf_dict['test AUC_mean']=0
    ...
    for jj in range(n_classes):
        ...
        perf_dict['valid AUC_%s'%jj]=roc_auc_score((class_inds_va)*1, y_valid_pred_proba_NC[:, jj])
        perf_dict['valid_AUPRC_%s'%jj]=average_precision_score((class_inds_va)*1, y_valid_pred_proba_NC[:, jj])        
    
        class_inds_te = y_test==jj
        perf_dict['test AUC_%s'%jj]=roc_auc_score((class_inds_te)*1, y_test_pred_proba_NC[:, jj])
        ...
        perf_dict['valid AUC_mean'] += perf_dict['valid AUC_%s'%jj]/n_classes
        perf_dict['test AUC_mean'] += perf_dict['test AUC_%s'%jj]/n_classes
...
#     y_train_N1, y_train_pred_proba_N1, train_auprc_1, train_roc_auc_1, y_train_N2, y_train_pred_proba_N2, train_auprc_2, train_roc_auc_2, y_train_N3, y_train_pred_proba_N3, train_auprc_3, train_roc_auc_3 = compute_binary_classification_perf(y_train, y_train_pred_proba_NC)
#     print('ROC AUC for class 1 on train : %.4f'%train_roc_auc_1)
#     print('AUPRC for class 1 on train : %.4f'%train_auprc_1)
    
#     print('ROC AUC for class 2 on train : %.4f'%train_roc_auc_2)
#     print('AUPRC for class 2 on train : %.4f'%train_auprc_2)
    
#     print('ROC AUC for class 3 on train : %.4f'%train_roc_auc_3)
#     print('AUPRC for class 3 on train : %.4f'%train_auprc_3)
    
    
#     y_valid_N1, y_valid_pred_proba_N1, valid_auprc_1, valid_roc_auc_1, y_valid_N2, y_valid_pred_proba_N2, valid_auprc_2, valid_roc_auc_2, y_valid_N3, y_valid_pred_proba_N3, valid_auprc_3, valid_roc_auc_3 = compute_binary_classification_perf(y_valid, y_valid_pred_proba_NC)
#     print('ROC AUC for class 1 on valid : %.4f'%valid_roc_auc_1)
#     print('AUPRC for class 1 on valid : %.4f'%valid_auprc_1)
    
#     print('ROC AUC for class 2 on valid : %.4f'%valid_roc_auc_2)
#     print('AUPRC for class 2 on valid : %.4f'%valid_auprc_2)
    
#     print('ROC AUC for class 3 on valid : %.4f'%valid_roc_auc_3)
#     print('AUPRC for class 3 on valid : %.4f'%valid_auprc_3)


#     y_test_N1, y_test_pred_proba_N1, test_auprc_1, test_roc_auc_1, y_test_N2, y_test_pred_proba_N2, test_auprc_2, test_roc_auc_2, y_test_N3, y_test_pred_proba_N3, test_auprc_3, test_roc_auc_3 = compute_binary_classification_perf(y_test, y_test_pred_proba_NC)
#     print('ROC AUC for class 1 on test : %.4f'%test_roc_auc_1)
#     print('AUPRC for class 1 on test : %.4f'%test_auprc_1)
    
#     print('ROC AUC for class 2 on test : %.4f'%test_roc_auc_2)
#     print('AUPRC for class 2 on test : %.4f'%test_auprc_2)
    
#     print('ROC AUC for class 3 on test : %.4f'%test_roc_auc_3)
#     print('AUPRC for class 3 on test : %.4f'%test_auprc_3)    
