...
----------------------
$ snakemake --snakefile train_gru_d_semi_supervised_for_los_ordinal_regression.smk --profile ../../utils/profiles/hugheslab_cluster/ train_and_evaluate_classifier_many_hyperparams train_and_evaluate_classifier_many_hyperparams

Train single hyperparam at a time
---------------------------------

To run with multiple random seeds (prespecified in a config file)
$ snakemake --cores 1 --snakefile train_gru_d_semi_supervised_for_los_ordinal_regression.smk train_and_evaluate_classifier_many_hyperparams
...
RESULTS_FEAT_PER_TSTEP_PATH=os.path.abspath("../results/GRUD/los_ordinal_regression")
...
rule train_and_evaluate_classifier:
    input:
        script=os.path.join(PROJECT_REPO_DIR, 'GRU_D', 'main_ordinal_eicu.py'),
